...
public class AccountService implements IAccountService {
  ...
  private void validateAccount(Account account) throws Exception {
    ...
    if (account.getAccountType().equals(AccountTypeEnum.SAVINGS)) {
      if (account.getMaintenanceFee() > 0.0) {
        throw new Exception(
            "Cuenta de ahorro debe ser libre de comisión por mantenimiento.");
      }

      if (account.getAccountMovementBoundaryType() == null || (account
          .getAccountMovementBoundaryType() != null
          && (!account.getAccountMovementBoundaryType()
              .getAccountMovementBoundaryFrequency().equals(
                  AccountMovementBoundaryFrequencyEnum.MOVEMENTBOUNDARY)
              || account.getAccountMovementBoundaryType()
                  .getMovementAmountBoundary() <= 0))) {
        throw new Exception(
            "Cuenta de ahorro debe tener limite de movimientos mensuales.");
      }
    }
    ...
    if (account.getAccountType()
      ...
      if (account.getMaintenanceFee() <= 0.0) {
        throw new Exception(
            "Cuenta corriente debe tener comisión por mantenimiento.");
      }

      if (account.getAccountMovementBoundaryType() != null && !account
          .getAccountMovementBoundaryType()
          .getAccountMovementBoundaryFrequency().equals(
              AccountMovementBoundaryFrequencyEnum.MOVEMENTBOUNDARY)
          && account.getAccountMovementBoundaryType()
              .getMovementAmountBoundary() > 0) {
        throw new Exception(
            "Cuenta corriente no debe tener limite de movimientos mensuales.");
      }
    ...
    if (account.getAccountType()
      ...
      if (account.getMaintenanceFee() >= 0.0) {
        throw new Exception(
            "Cuenta a plazo fijo debe ser libre de comisión por mantenimiento.");
      }
    ...
    // return Flux.error(
    // new Exception("Un cliente personal solo puede tener un " +
    // "máximo de una cuenta de ahorro, una cuenta " +
  ...
  @Override
  public Mono<Account> save(String id, Account account)
      throws Exception {
    return this.accountRepository.save(account);
  }
